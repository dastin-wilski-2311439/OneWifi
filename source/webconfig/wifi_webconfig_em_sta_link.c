...
webconfig_subdoc_object_t   em_sta_link_objects[3] = {
    { webconfig_subdoc_object_type_version, "Version" },
    { webconfig_subdoc_object_type_subdoc, "SubDocName" },
    { webconfig_subdoc_object_type_em_sta_link, "WifiStaLink" },
};

webconfig_error_t init_em_sta_link_subdoc(webconfig_subdoc_t *doc)
{
    doc->num_objects = sizeof(em_sta_link_objects)/sizeof(webconfig_subdoc_object_t);
    memcpy((unsigned char *)doc->objects, (unsigned char *)&em_sta_link_objects, sizeof(em_sta_link_objects));
...
webconfig_error_t encode_em_sta_link_subdoc(webconfig_t *config, webconfig_subdoc_data_t *data)
{
    ...
    cJSON_AddStringToObject(json, "SubDocName", "Easymesh STA link");

    obj_emstalink = cJSON_CreateObject();
    cJSON_AddItemToObject(json, "WifiStaLink", obj_emstalink);


    if (encode_em_sta_link_object(&params->em_config, obj_emstalink) != webconfig_error_none) {
