...
static int prepare_sta_lins_metrics_data(webconfig_subdoc_data_t *data, client_assoc_data_t *stats)
{
    ...
    data->u.decoded.em_sta_link_metrics_rsp.per_sta_metrics = (per_sta_metrics_t *)malloc(sta_count * sizeof(per_sta_metrics_t));
    ...
    for (int i = 0; i < MAX_NUM_VAP_PER_RADIO; i++)
    {
        for (int j = 0; j < stats[i].stat_array_size; j++)
        {
            if (stats[i].threshold_hit[j] == true)
            {
                ...
                memcpy(param[sta_it].assoc_sta_link_metrics.assoc_sta_link_metrics_data[0].bssid, stats[i].assoc_stats[j].link_mac, sizeof(mac_address_t));
                ...
                memcpy(param[sta_it].assoc_sta_ext_link_metrics.assoc_sta_ext_link_metrics_data[0].bssid, stats[i].assoc_stats[j].link_mac, sizeof(mac_address_t));
